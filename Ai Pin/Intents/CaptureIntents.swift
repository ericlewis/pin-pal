...
//extension CaptureEntity: EntityPropertyQuery {
    //
    //    public static var properties = EntityQueryProperties<CaptureEntity, Predicate<CaptureEntity>> {
    //        Property(\.$createdAt) {
    ...
    //        matching comparators: [Predicate<CaptureEntity>],
    //        mode: ComparatorMode,
    //        sortedBy: [EntityQuerySort<CaptureEntity>],
    //        limit: Int?
    //    ) async throws -> Self.Result {
    //        let content: [ContentEnvelope] = try await service.captures(0, limit ?? 1000).content
    //        var filtered = try await content.concurrentCompactMap { content in
    //            let entity = await CaptureEntity(from: content)
